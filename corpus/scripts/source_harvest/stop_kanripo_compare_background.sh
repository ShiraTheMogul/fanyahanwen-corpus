...
if [[ ! "$pid" =~ ^[0-9]+$ ]] || ! kill -0 "$pid" 2>/dev/null; then echo "No Kanripo comparison job is running."; exit 0; fi
echo "Stopping Kanripo comparison session $pid ..."
kill -TERM -- "-$pid" 2>/dev/null || kill -TERM "$pid" 2>/dev/null || true
for _ in $(seq 1 20); do if ! kill -0 "$pid" 2>/dev/null; then echo "Stopped cleanly."; exit 0; fi; sleep 1; done
