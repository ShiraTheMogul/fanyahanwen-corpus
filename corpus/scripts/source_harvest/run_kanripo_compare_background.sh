...
if [[ -f "$PID_FILE" ]]; then
  ...
  if [[ "$old_pid" =~ ^[0-9]+$ ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "A Kanripo comparison job is already running as PID $old_pid."
    echo "Use: bash $SCRIPT_DIR/kanripo_compare_status.sh"
    exit 1
  fi
  rm -f "$PID_FILE"
fi
...
command=(
  "${priority[@]}"
  python3 "$SCRIPT_DIR/kanripo_witness_compare.py"
  --repo-root "$REPO_ROOT"
...
if ! kill -0 "$pid" 2>/dev/null; then
  echo "Kanripo comparison exited immediately. Log: $log_file"
