...
title, authors, and a `documents` list. That is the label source; the 54MB
`index_corpus.csv` is not needed and is never touched.

Two notes on fields
-------------------
`body_start_line` in `documents[]` is a dead artifact — in `clean/` the body
starts at line 1 and the field disagrees. It is ignored here. `tools/
strip_body_start_line.py` in this package removes it from the metadata files if
you want it gone at source.

Files in `clean/` are UTF-8 without BOM. Everything here reads with `utf-8-sig`,
so BOM and no-BOM both work and neither the reader nor your corpus has to change.

The evaluation that actually matters
------------------------------------
`group_key` exists so you can hold out an entire tradition. Train on 中國漢文
alone, test on 日本漢文 / 朝鮮漢文 / 越南漢文: if the scorer generalises across
that split it is measuring Literary Chinese, and if it does not, it has learned
to recognise Chinese authorship and needs to be rebuilt. That is the experiment
that tells you whether the thing works, and no aggregate accuracy number can
substitute for it.
"""
