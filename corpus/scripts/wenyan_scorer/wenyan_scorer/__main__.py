...
CLI.
    ...
    python -m wenyan_scorer score FILE...          score text files
    python -m wenyan_scorer rules                  list the ruleset with citations
    python -m wenyan_scorer explain RULE_ID        show one rule in full
    python -m wenyan_scorer calibrate ...          fit weights against labelled text
    python -m wenyan_scorer corpus-stats ROOT      what is in a corpus subtree
"""
...
def cmd_explain(args) -> int:
    ...
    if r is None:
        ...
        print("run `python -m wenyan_scorer rules` to list them", file=sys.stderr)
        return 1
    print(f"{r.rule_id}")
