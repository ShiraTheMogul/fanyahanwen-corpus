...
This ruleset exists so that "not Literary Chinese" can be asserted on positive
evidence. It must never be satisfiable by a text merely failing to use
Pulleyblank's constructions, because the corpus is full of perfectly good 漢文
that does not use them — short inscriptions, verse, parataxis-heavy narrative,
and the Japanese, Korean, Vietnamese and Ryukyuan traditions writing in their own
idiom. So every rule matches a grammatical morpheme doing Mandarin work: a
structural particle, an aspect suffix, a classifier frame, a paired conjunction.
None matches "absence of 之" or "short sentences".

The trap is that almost every Mandarin grammatical morpheme is written with a
graph that has a perfectly good Literary life as a content word:
...
Matching the bare graph would mark Literary Chinese down for using ordinary
Literary Chinese words. Every rule below matches the morpheme in a position only
...
RULES = [
    ...
    # REMOVED: vn.adv.de_adverbial (X 地 + V). It fired 12 times, all on 地
    # 'land' as an ordinary Literary noun — 此地磧礫甚鮮美, 其地瓦屋鱗比,
    # 北極出地二十六度. Telling the suffix from the noun needs to know that what
    # precedes is a disyllabic state expression and what follows is a verb,
    # which is a parse rather than a regex.
    ...
    Rule(
        ...
        # One anchor group only. The verb restriction is a fixed-width
        # lookbehind so the anchor stays on 著; with two alternative named
        # groups, `m.start("anchor")` returns -1 for whichever did not
        # participate, collapsing every such match onto one dedupe key.
    ...
    # REMOVED: vn.asp.guo (experiential 過). It fired in 56,541 documents (19%
    # of the corpus), 92% of which stayed literary. In 402,000 characters of
    # unambiguous Literary Chinese (全唐文, 康熙朝實錄) all 33 hits were the noun
    # 過 'fault' — 極言朕過, 改過, 補過, 無過. The rule keyed on 過 before a clause
    # boundary, which is exactly where Literary Chinese puts that noun.
    ...
    # REMOVED: vn.arg.zai_preverbal (在 + place + V). It fired 3 times, all on
    # Literary 在 as a main verb taking a locative — 在海峽之中央小孤嶼也,
    # 巳在都下灣泊矣. The opposition is real (Pulleyblank sets Literary postverbal
    # 於 against modern preverbal coverbs, V.6a.i p.48) but is only detectable
    # from the Literary side, which lc.coverb.yu_locative already covers.
    ...
    # REMOVED: vn.link.paired_hypotaxis, a bare list of vernacular
    # subordinators. All 8 hits were false. 所以 is Literary 'that by which'
    # (Pulleyblank V.6a.i p.50 insists it never means 'therefore' here), so it
    # now scores on the literary side as lc.nom.suo_coverb; 雖然 is Literary
    # 'though it is so', a full clause (XV.3a p.157). 而且, 但是, 可是, 只有 and
    # 並且 decompose into ordinary Literary sequences too. Only the complete
    # paired frame below survives.
