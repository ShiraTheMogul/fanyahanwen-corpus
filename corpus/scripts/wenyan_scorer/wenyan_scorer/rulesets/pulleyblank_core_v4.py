...
These are constructions of the core inherited grammar, which every 漢文 tradition
writes on top of, whether the author was in Luoyang, Kyoto, Hanseong, Thang Long,
Shuri, Malacca, or writing 維基大典 last week. Nothing here requires a text to
look Warring States or to be Chinese. A rule that fired only on pre-Qin Chinese
usage would be measuring period, and would mark 日本漢文 or 越南漢文 down for not
being the Mencius.

Every rule here is polarity 'lc', so it is positive evidence only. The absence of
these constructions is evidence of nothing: short texts, parataxis-heavy verse
and plain narrative all lack most of them. Only the vernacular ruleset can push a
text away from Literary Chinese.
...
`HAN` is one Han character across every CJK block (see utils.HAN_BODY).
...
RULES = [
    ...
    Rule(
        ...
        "by anchor — in an earlier version this rule fired once per starting offset and a single "
