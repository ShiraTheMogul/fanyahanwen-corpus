"""
Full-corpus scan: walk every .txt, score it, stream the results to JSONL.

The run is long enough that it has to survive itself, so it is resumable
(results are appended as they are produced, and a re-run skips every path
already in the output file), streaming (enumeration uses os.walk and yields as
it goes, rather than materialising a sorted listing of a quarter of a million
paths first), and parallel (`--workers` defaults to one per CPU; reading and
scoring cost about the same, so both are worth overlapping). It reports its own
throughput every 500 documents.

It walks .txt rather than metadata.json because metadata does not reliably
enumerate documents: 四庫全書 has 2,740 metadata files against 94,626 .txt,
while 維基大典 and 礦藝大典 have more metadata than text, so around 1,800 works
there have metadata and no body. Each text is attached to its nearest ancestor
metadata.json where one exists.

`--index` drives the walk from index_corpus.csv instead, which is much faster
for 中國漢文 and 朝鮮漢文. It is not the default because the index is stale: it
holds no rows at all for 四庫全書, 維基大典, 礦藝大典 or 琉球漢文, and is missing
something like 104,000 documents.
"""
