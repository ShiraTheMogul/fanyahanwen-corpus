"""
Turn a scan into something a person can actually read.

Three outputs:

  `flagged.csv`   one row per not_literary / uncertain / too_short document,
                  with the rules that fired and a snippet of the actual text.
                  This is your "what".

  `rules.csv`     which vernacular rules are driving rejections, ranked, with
                  example matches. **This is where false positives show up.**
                  If one rule accounts for 6,000 of 7,648 rejections, that rule
                  is the story and the other 114 are noise. This is your "why".

  `roots.csv`     label counts per corpus root and period, for the overview.
    ...
    with open(rules_path, "w", encoding="utf-8-sig", newline="") as fh:
        ...
        for rid, ndocs in rule_docs.most_common():
            ...
            w.writerow([
                ...
                # THE column to read. A rule that fires in tens of thousands of
                # documents that are still classified literary is matching
                # Literary Chinese. Both figures below have the right
                # denominator; an earlier version divided a count taken over all
                # documents by the number of flagged ones and printed "576%".
