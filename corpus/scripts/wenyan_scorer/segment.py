...
The window mode exists because much premodern material carries no punctuation at
all. Note that segmenting is only half of that problem: rules whose patterns
depend on a punctuation mark cannot fire on such a text either, which is why
`Rule.requires_punctuation` exists and why the scorer reports how many rules it
had to skip rather than letting them contribute zippo, nada, AND goose egg.
"""
