...
# -*- coding: utf-8 -*-
"""
Nested-unknown regionalisation helper for staging.
...
---------------------
A file inside a nested 不詳 folder is treated as unknown *inside that current historical container*.
...
The script works without those packages if you do not use --fetch-ws-categories or --author-lookup.
...
Only rows supplied are used for non-existing target folders. The script does not invent states.
...
SCRIPT_VERSION = "regionalise_three_periods_2026-06-17_windows_path_safe"
...
def safe_filename(name: str, max_len: int = 120) -> str:
    ...
    this does not add helpful suffixes such as "..." or "__dup2".
    It only removes/replaces characters that can make the output unwritable.

    Windows treats path components ending in a dot or space as invalid/unstable,
    so strip those after truncation. This fixes old scraped folders such as
    "壬寅二月有詔令..." without inventing a duplicate-style filename.
    """
    name = (name or "").strip()
...
def write_text_file(path: Path, text: str) -> None:
    ...
    try:
        ...
        raise FileNotFoundError(
            f"Could not write output file. This is usually a Windows path-component problem. "
            f"Path was: {path}"
        ) from exc
...
def build_header(meta: Dict[str, str]) -> str:
    """Build a corpus header using approved keys only.

    Important: this intentionally does NOT pad files with blank placeholder
    fields. It writes an approved key only when the value is non-empty.
    Audit/provenance fields still never enter corpus files.
    """
    meta2 = sanitize_header_meta(meta)
    rows = []
    for key in HEADER_KEYS:
        value = str(meta2.get(key, "") or "").strip()
        if value:
...
class WikiClient:
    ...
    def discover_subpages_from_root_html(self, root_title: str) -> List[str]:
        html = self.fetch_html(root_title)
        ...
        for a in body.select("a[title]"):
            ...
            if t.startswith(prefix) and t not in out:
                out.append(t)
        return sorted(out, key=lambda t: chapter_sort_key(root_title, t))

    def discover_work_parts(self, root_title: str, *, method: str = "both") -> List[str]:
        root_title = canonical_root_from_title(root_title)
        prefix = root_title + "/"
        out: List[str] = []
        if method in {"html", "both", "full"}:
            out.extend(self.discover_subpages_from_root_html(root_title))
        if method in {"links", "both", "full"}:
            out.extend(t for t in self.fetch_links(root_title) if t.startswith(prefix))
        if method in {"allpages", "full"}:
            out.extend(self.fetch_allpages_prefix(prefix))
        clean: List[str] = []
        bad_tail_patterns = ("/doc", "/sandbox", "/沙盒")
        for t in sorted(set(out), key=lambda x: chapter_sort_key(root_title, x)):
    ...
    def fetch_author_description(self, author: str) -> str:
        ...
        for bad in root.select("script, style, table, .mw-editsection, .toc, .catlinks, .printfooter, .ws-noexport, .noprint"):
            bad.decompose()
...
def chapter_sort_key(root_title: str, page_title: str) -> Tuple[int, int, str]:
    if page_title == root_title:
        return (0, 0, "root")
    label = page_title[len(root_title) + 1:] if page_title.startswith(root_title + "/") else page_title
    last = label.split("/")[-1]
...
def html_to_text_inline_preserving(root: Any) -> str:
    parts: List[str] = []

    def walk(node: Any) -> None:
        if BeautifulSoup is None:
            return
        from bs4 import Comment, NavigableString, Tag  # type: ignore
...
def remove_wikisource_furniture(root: Any) -> None:
    for selector in [
...
def strip_wikisource_parser_report(text: str) -> str:
    """Remove MediaWiki parser/cache diagnostics if they leak from comments.

    This specifically targets blocks beginning with "NewPP limit report" and
    related render/cache lines. These are never corpus text.
    """
    if not text:
        return text
    markers = [
...
def clean_existing_body_text(body: str) -> str:
    """Minimal cleanup for existing corpus bodies before rewriting.

    This does not modernise or normalise corpus prose. It only removes obvious
    Wikisource parser diagnostics that should never have entered a text file.
    """
    body = strip_wikisource_parser_report(body or "")
    return body.strip() + ("\n" if body.strip() else "")
...
def clean_html_to_text(html: str) -> str:
    ...
    text = strip_wikisource_parser_report(text).strip()
...
def canonical_root_from_title(title: str) -> str:
    title = (title or "").strip()
    if "/" in title:
...
def choose_root_title_for_discovery(group: WorkGroup) -> str:
    ...
    for raw in [wikisource_url_to_title(sample.meta.get("SOURCE_URL", "")), sample.page_title, sample.work_title]:
        raw = (raw or "").strip()
        if raw:
            return canonical_root_from_title(raw)
    return canonical_root_from_title(work_folder_name_from_group(group))
...
def discovered_part_filename(root_title: str, page_title: str, seq: int) -> str:
    """Filename for an online-discovered missing part.

    Do not add _rescraped or duplicate-number tags. Those suffixes make
    replacement workflows unsafe. If this name collides, the caller skips and
    writes an audit row rather than inventing a new filename.
    """
    chapter = page_title[len(root_title) + 1:] if page_title.startswith(root_title + "/") else page_title
    if not chapter or chapter == root_title:
        chapter = "front_matter"
    return f"{safe_filename(root_title)}__{safe_filename(chapter)}.txt"
...
def discover_existing_target_paths(records: Sequence[FileRec]) -> List[str]:
    targets = set()
    for r in records:
        rel_without_root = "/".join(r.parts[1:-2]) if len(r.parts) >= 3 else ""
        if rel_without_root and UNKNOWN_LABEL not in rel_without_root.split("/"):
            targets.add(rel_without_root)
...
def normalise_target_roots(raw: str | Sequence[str] | None) -> List[str]:
    if raw is None:
        return []
...
def collect_preservable_dir_skeleton(input_path: Path, target_roots: Sequence[str]) -> Set[str]:
    """Collect structural directories that should be recreated in output.

    This supports a container input holding several target period folders, e.g.
    宋朝/, 南北朝/, 五代十國/. Only target_roots are considered.
    """
    entries, _is_zip = _source_entry_parts_for_skeleton(input_path)
    target_roots = normalise_target_roots(target_roots)
    if not entries:
        return set(target_roots)

    top_parts = {parts[0] for parts, _is_dir in entries if parts}
    input_root_name = input_path.stem if input_path.is_file() else input_path.name

    if target_roots and top_parts.intersection(target_roots):
        mode = "container_has_targets"
    elif input_root_name in target_roots:
        mode = "input_is_target_root"
    else:
        raise SystemExit(
            "Input does not appear to contain the requested target roots. "
            f"Found top-level folders: {', '.join(sorted(top_parts))}; "
            f"target roots: {', '.join(target_roots)}"
        )
    ...
    for raw_parts, is_dir in entries:
        if not raw_parts:
            continue
        if mode == "container_has_targets":
            if raw_parts[0] not in target_roots:
                continue
            parts = raw_parts
        else:
            parts = [input_root_name] + raw_parts
...
def load_records(input_path: Path, target_roots: Sequence[str]) -> Tuple[List[str], List[FileRec]]:
    """Load text records from a folder or zip without extracting long paths.

    The zip path reader is deliberate: some Song titles are longer than normal
    filesystem limits, even on Linux temp dirs. Reading zip members directly
    avoids creating those long temporary paths.
    """
    target_roots = normalise_target_roots(target_roots)
    input_root_name = input_path.stem if input_path.is_file() else input_path.name
    ...
    if input_path.is_file() and input_path.suffix.lower() == ".zip":
        with zipfile.ZipFile(input_path) as z:
            for info in z.infolist():
                if info.is_dir():
                    continue
                rel = PurePosixPath(info.filename).as_posix().strip("/")
        ...
        folder_paths = [p for p in input_path.rglob("*.txt") if not p.name.endswith(BACKUP_SUFFIXES)]
    ...
    if not zip_infos and not folder_paths:
        raise SystemExit(f"No .txt files found under {input_path}")

    if target_roots and top_parts.intersection(target_roots):
        mode = "container_has_targets"
        active_roots = sorted(top_parts.intersection(target_roots), key=target_roots.index)
    elif input_root_name in target_roots:
        mode = "input_is_target_root"
        active_roots = [input_root_name]
    else:
        raise SystemExit(
            "Input does not appear to contain the requested target roots. "
            f"Found top-level folders: {', '.join(sorted(top_parts))}; "
    ...
    def add_record(rel_from_source: str, text: str, source_mtime_sort: float, source_mtime_utc: str) -> None:
        raw_parts = list(PurePosixPath(rel_from_source).parts)
        if not raw_parts:
            return
        if mode == "container_has_targets":
            if raw_parts[0] not in active_roots:
                return
            parts = raw_parts
            rel = rel_from_source
            candidate_root = raw_parts[0]
        else:
            parts = [input_root_name] + raw_parts
            rel = "/".join(parts)
            candidate_root = input_root_name

        meta, body = parse_header_and_body(text)
        body = clean_existing_body_text(body)
        work_dir = parts[-2] if len(parts) >= 2 else safe_filename(meta.get("WORK_TITLE", PurePosixPath(rel_from_source).stem))
    ...
    if input_path.is_file() and input_path.suffix.lower() == ".zip":
        with zipfile.ZipFile(input_path) as z:
            for info in sorted(zip_infos, key=lambda i: i.filename):
                rel_from_source = PurePosixPath(info.filename).as_posix().strip("/")
                source_mtime_sort, source_mtime_utc = zipinfo_mtime(info)
                try:
                    raw = z.read(info)
                    txt = raw.decode("utf-8")
                except UnicodeDecodeError:
                    txt = raw.decode("utf-8", errors="replace")
                add_record(rel_from_source, txt, source_mtime_sort, source_mtime_utc)
    else:
        for p in sorted(folder_paths):
            rel_from_source = p.relative_to(input_path).as_posix()
            try:
            ...
            add_record(rel_from_source, read_text_file(p), source_mtime_sort, source_mtime_utc)

    return active_roots, records
...
def top_rel_after_root(group_rel: str) -> str:
...
def allowed_target_for_unknown(rec: FileRec, target_path: str, *, minor_folder: str, allow_minor_from: str) -> Tuple[bool, str]:
    ...
    # Root-level 不詳, e.g. 晉朝/不詳, may move to any target below the root.
    if not context:
        return True, "root-unknown"
...
def normalised_output_text(rec: FileRec, *, nation: str, author: str = "", ws_categories: str = "", source_url: str = "") -> str:
    ...
    cleaned_body = clean_existing_body_text(rec.body)
    text_out = build_header(meta) + cleaned_body
    assert_approved_header_only(text_out)
    return text_out


def output_rel_for_rec(root_name: str, target_path: str, work_folder: str, file_name: str) -> str:
    parts = [safe_filename(root_name)]
    if target_path:
        parts.extend([safe_filename(p) for p in target_path.split("/") if p])
    parts.append(safe_filename(work_folder))
    parts.append(safe_filename(file_name, max_len=180))
    return "/".join(parts)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    ap.add_argument("--target-roots", default="宋朝,南北朝,五代十國", help="Comma-separated top-level period folders to process. Default: 宋朝,南北朝,五代十國")
    ...
    ap.add_argument("--discover-missing-parts", action="store_true", help="Check root/table-of-contents pages for missing Wikisource subpages and write audit rows. By default this does not save new corpus files.")
    ap.add_argument("--write-discovered-parts", action="store_true", help="Actually save newly discovered Wikisource parts. Default is audit-only to avoid surprise output-shape changes.")
    ...
    log_progress(args, f"[schema] corpus header keys restricted to: {', '.join(HEADER_KEYS)}; blank placeholders are not added", verbose_only=True)
    ...
    target_roots = normalise_target_roots(args.target_roots)
    root_names, records = load_records(input_path, target_roots)
    input_dir_skeleton = collect_preservable_dir_skeleton(input_path, root_names)
    existing_targets = discover_existing_target_paths(records)
    log_progress(args, f"[load] roots={','.join(root_names)} records={len(records)} existing_target_paths={len(existing_targets)} skeleton_dirs={len(input_dir_skeleton)}")
    ...
    if args.preserve_empty_dirs:
        ...
        for rule in rules:
            if rule.target_path:
                # Region-map targets are relative to each processed root unless
                # the row already begins with an explicit processed root name.
                bits = [p for p in rule.target_path.split("/") if p]
                if bits and bits[0] in root_names:
                    preserved_dirs.add("/".join(bits))
                else:
                    for rn in root_names:
                        preserved_dirs.add("/".join([rn] + bits))
    ...
    for title_i, (title_key, same_title_groups) in enumerate(groups_by_title.items(), start=1):
        ...
        for unk in unknown_groups:
            ...
            for named in named_groups:
                named_target = top_rel_after_root(named.group_rel)
            ...
            allowed_targets = sorted({top_rel_after_root(g.group_rel) for g in allowed_named})
    ...
    for unknown_i, group in enumerate(unknown_candidates, start=1):
        ...
        if target:
            ...
            keep_target = top_rel_after_root(group.group_rel)
            if UNKNOWN_LABEL in keep_target.split("/"):
                ...
                keep_nation = container_parts[-1] if container_parts else sample.root
            else:
                keep_nation = keep_target.split("/")[-1] if keep_target else sample.root
    ...
    for write_i, group in enumerate(sorted_write_groups, start=1):
        ...
        if target_info:
            ...
            target_path = top_rel_after_root(group.group_rel)
        ...
        for rec in sorted(group.files, key=lambda r: r.rel_path):
            if rec.is_unknown and not target_info:
                ...
                nation_for_header = rec.meta.get("NATION") or (container_parts[-1] if container_parts else rec.root)
            else:
                out_rel = output_rel_for_rec(rec.root, target_path, work_folder_name_from_group(group), rec.parts[-1])
                nation_for_header = nation or rec.meta.get("NATION") or (target_path.split("/")[-1] if target_path else rec.root)

            # Do not invent duplicate-number filenames. If two different
            # bodies want the same output path, skip the second and put the
            # conflict in the manual/rescrape queue.
            final_rel = out_rel
            if final_rel in written_paths and written_paths[final_rel] != rec.sha1:
                skipped_files += 1
                row = {
                    "priority": "high",
                    "review_type": "output_path_collision",
                    "suggested_action": "manual_review_before_write; do_not_invent_dup_filename",
                    "reason": "same_output_path_different_body",
                ...
                manual_review_rows.append(row)
                rescrape_rows.append(dict(row, rescrape_reason="output_path_collision"))
                file_audit_rows.append({
                    "source_rel": rec.rel_path,
                    "output_rel": final_rel,
                    "status": "skipped_output_path_collision",
                    "work_title": rec.work_title,
                    "page_title": rec.page_title,
    ...
    # Fourth pass: optional root/table-of-contents discovery for missing subpages.
    # This is intentionally separate from regionalisation: it only adds pages under
    # the same Wikisource root title, e.g. WORK/卷一, WORK/卷二. It does not follow
    # arbitrary links.
    if args.discover_missing_parts and wiki is not None:
        log_progress(args, "[pass 4/4] discover missing Wikisource subpages under same root titles")
        ...
        for discover_i, group in enumerate(discovery_groups, start=1):
            ...
            sample = group.files[0]
            root_title = choose_root_title_for_discovery(group)
            if not root_title:
                log_progress(args, f"[discover:skip] {group.group_rel}; no root title", verbose_only=True)
                continue
            checked_discovery_groups += 1
            existing_titles = existing_titles_for_group(group)
            log_progress(args, f"[discover] {group.group_rel}; root={root_title}; existing_titles={len(existing_titles)} method={args.discover_parts_method}", verbose_only=True)
            try:
                discovered = wiki.discover_work_parts(root_title, method=args.discover_parts_method)
                log_progress(args, f"[discover] {root_title}: discovered={len(discovered)}", verbose_only=True)
            except Exception as exc:
            ...
            log_progress(args, f"[discover] {root_title}: missing={len(missing)}", verbose_only=True)
            ...
            if group_is_unknown and not target_info:
                ...
                nation_for_header = sample.meta.get("NATION") or (container_parts[-1] if container_parts else sample.root)
            else:
                nation_for_header = nation or sample.meta.get("NATION") or (target_path.split("/")[-1] if target_path else sample.root)

            enrich = enrichment_for_group.get(group.group_rel, {})
            for idx, page_title in enumerate(missing, start=1):
                if args.dry_run or not args.write_discovered_parts:
                    status = "would_save_dry_run" if args.dry_run else "audit_only_not_written"
                    log_progress(args, f"[discover:audit] {status} {page_title}", verbose_only=True)
                    discovered_part_rows.append({
                        "group": group.group_rel,
                        "root_title": root_title,
                        "page_title": page_title,
                ...
                try:
                    ...
                    if not cats and page_title != root_title:
                        cats = wiki.fetch_categories(root_title)
                except Exception as exc:
                    discovered_part_rows.append({
                        "group": group.group_rel,
                        "root_title": root_title,
                ...
                fname = discovered_part_filename(root_title, page_title, idx)
                out_rel = output_rel_for_rec(sample.root, target_path, work_folder_name_from_group(group), fname)
                final_rel = out_rel
                ...
                if final_rel in written_paths and written_paths[final_rel] != body_sha:
                    ...
                    manual_review_rows.append({
                        "priority": "high",
                        "review_type": "discovered_part_output_path_collision",
                        "suggested_action": "manual_review_before_write; do_not_invent_dup_filename",
                        "reason": "same_output_path_different_body",
                ...
                meta["WORK_TITLE"] = root_title
                meta["DISPLAY_TITLE"] = meta.get("DISPLAY_TITLE") or root_title
                meta["PAGE_TITLE"] = page_title
                meta["AUTHOR"] = enrich.get("AUTHOR", "") or meta.get("AUTHOR", "")
                meta["NATION"] = nation_for_header
                meta["CATEGORIES"] = meta.get("CATEGORIES", "")
                meta["YEAR"] = meta.get("YEAR", "")
                meta["CHAPTER"] = page_title[len(root_title) + 1:] if page_title.startswith(root_title + "/") else page_title
    ...
    summary = {
        ...
        "write_discovered_parts": bool(args.write_discovered_parts),
