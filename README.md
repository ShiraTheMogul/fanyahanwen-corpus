...
# Authority data maintenance

Automatic person/place/office annotation uses the installed CBDB SQLite as a read-only authority source plus a project-maintained Shang/Xia workbook at `viewer/data/shang_people.xlsx`. The workbook is the source of truth for the supplementary people; its generated SQLite index under the existing corpus-search cache is disposable. Replacing the workbook and rebuilding the authority indexes is sufficient.

The supplementary importer reads the `Shang` and `Xia` sheets with these columns: `person_id`, `primary_name`, `name_han`, `aliases`, `polity`, `period`, `year_start`, `year_end`, `date_label`, `roles`, `places`, `source_citations`, `external_ids`, and `notes`. `person_id` is local to its sheet, so `Shang:1` and `Xia:1` remain distinct authority records. Blank scholarly fields stay blank.

At rebuild time, explicit `name_han` and `aliases` are expanded through the same broad character-equivalence registry used by Corpus Search. This permits OpenCC simplified/traditional mappings, Japanese shinjitai, and indexed historical variants to act as derived search aliases. Derived forms are never written back into the workbook and never merge authority records; chronology and context still resolve collisions.

One-character Shang 巫 entries are deliberately stricter. Automatic annotation requires the oracle-bone divination syntax `卜 … X貞` or punctuation-separated `X，貞` (modern simplified `贞` is also accepted). These one-character diviner matches are suppressed in post-Shang premodern material and become eligible again in RoC/PRC-era historical or archaeological discussion.

Maintenance commands, from `viewer/`:

```bash
bin/rails authority:status
bin/rails authority:refresh_and_build
bin/rails cbdb:verify
bin/rails corpus_search:rebuild_manifest
```

`authority:refresh_and_build` checks CBDB, rebuilds its small name-pointer cache when necessary, and rebuilds the supplementary Shang/Xia index when the workbook or the character-equivalence mappings change. `corpus_search:rebuild_manifest` invokes that authority refresh as a prerequisite.
