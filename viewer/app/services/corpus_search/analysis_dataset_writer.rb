...
module CorpusSearch
  ...
  class AnalysisDatasetWriter
    COLUMNS = %w[
      doc_id document_id work_id body_fingerprint duplicate_group_size representative_document_id duplicate_members_json path folder_path document_role canonical_parent_path
    ...
    def row_for(record)
...

      {
        ...
        "duplicate_group_size" => record.duplicate_group_size.to_i,
        "representative_document_id" => record.representative_document_id,
        "duplicate_members_json" => JSON.generate(record.duplicate_members),
        "path" => doc["path"],
