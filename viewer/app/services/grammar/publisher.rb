...
module Grammar
  class Publisher
    ...
    def publish!(entry_id:, locale:, proposed_markdown:, credit:)
      ...
      metadata["contributors"] = merge_contributors(
        Array(existing&.metadata&.dig("contributors")),
        credit
      )
    ...
    def merge_contributors(existing, credit)
      rows = Array(existing).filter_map do |item|
        item.is_a?(Hash) ? MarkdownDocument.stringify_keys(item) : nil
      end

      public_credit = MarkdownDocument.stringify_keys(credit.to_h)
    ...
    def atomic_write(path, content)
      ...
      Tempfile.create(["grammar-entry", ".md"], path.dirname.to_s) do |file|
        file.binmode
        file.write(content)
        file.flush
        file.fsync
